...
from mitsbot_globals import MITS_COLOR, discord, resourcesEmbed, addEventListener
...
# function ran when resources are requested
async def sendResourcesResponse(message):
    await message.channel.send(embed=resourcesEmbed)


# add event listeners for resources command
addEventListener("resources", sendResourcesResponse)
addEventListener("resource", sendResourcesResponse)
addEventListener("links", sendResourcesResponse)
